...
{      for(int aj=-6;aj<=6;aj++)
      {
        double cost = pow((pow(ai,2)+pow(aj,2)),0.5);
        node dummy = node(current.x + (current.vx*1)+ ai/2,current.y + (current.vy*1)+aj/2,current.vx+ai,current.vy+aj,cost ,cid);
